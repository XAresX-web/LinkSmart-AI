...
async function handleSubscriptionChange(subscription: Stripe.Subscription) {
  ...
  if (subscription.items.data[0]?.price.id === "price_pro_monthly") {
    ...
    subscription.items.data[0]?.price.id === "price_business_monthly"
  ) {
    planType = "business";
  }
...
async function handlePaymentSuccess(invoice: Stripe.Invoice) {
  ...
  await supabase.from("notifications").insert({
    ...
    message: `Tu pago de €${(invoice.amount_paid / 100).toFixed(
      2
...
async function handlePaymentFailure(invoice: Stripe.Invoice) {
  ...
  await supabase.from("notifications").insert({
    ...
    message:
      "Hubo un problema procesando tu pago. Por favor, actualiza tu método de pago.",
    type: "error",
